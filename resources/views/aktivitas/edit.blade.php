        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Ubah Data Aktivitas</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <form role="form" action="/vaktivitas/{{$aktivitas->id}}/update" method="POST">
              {{csrf_field()}}
                <!-- text input -->
                ...
                <option value="{{$dinas->id}}" {{ ( $dinas->id == $aktivitas->dinas_id) ? 'selected' : '' }}>{{$dinas->nama_dinas}} </option>
                @endforeach
                      </select>
                      <select name="bidang_id" class="form-control" id="dinas" style="margin-bottom: 10px;">
                        <option disabled selected>Pilih Bidang</option>
                        @foreach($data_bidang as $bidang)
                <option value="{{$bidang->id}}" {{ ( $bidang->id == $aktivitas->bidang_id) ? 'selected' : '' }}>{{$bidang->nama_bidang}} </option>
                @endforeach
                      </select>
                ...
                <option value="{{$seksi->id}}" {{ ( $seksi->id == $aktivitas->seksi_id) ? 'selected' : '' }}>{{$seksi->nama_seksi}} </option>
                @endforeach
                      </select>
                    </div>
                <div class="form-group">
                  <label>Nama Aktivitas</label>
                  <input type="text" name="nama_aktivitas" class="form-control" value="{{$aktivitas->nama_aktivitas}}"  required>
                </div>
                <div class="form-group">
                  <label>Kepala Aktivitas</label>
                  <input type="text" name="kepala_aktivitas" class="form-control" value="{{$aktivitas->kepala_aktivitas}}"  required>
                </div>
