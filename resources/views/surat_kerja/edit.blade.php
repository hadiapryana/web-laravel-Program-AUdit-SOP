        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Ubah Data Surat Kerja</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <form role="form" action="/vsurat/{{$surat_kerja->id}}/update" method="POST">
              {{csrf_field()}}
                <!-- text input -->
                <div class="form-group">
                            <label >Nomor Surat</label>
                            <input name="no_surat" type="text" class="form-control"  value="{{$surat_kerja->no_surat}}" placeholder="No Surat" required>
                        </div>
                        <div class="form-group">
                            <label >tanggal Penugasan</label>
                            <input name="tgl_penugasan" type="date" class="form-control" value="{{$surat_kerja->tgl_penugasan}}" placeholder="tgl_penugasan" required>
                        </div>
                        <div class="form-group">
                            <label >Nama Pengawas</label>
                            <input name="nama_pengawas" type="text" class="form-control" value="{{$surat_kerja->nama_pengawas}}" placeholder="Nama Pengawas" required>
                        </div>
                        <label >Pilih Nama SOP</label>
                        <select  name="sop_id" class="form-control" id="sop" style="margin-bottom: 10px;">
                      @foreach($data_sop as $sop)
                             <option value="{{$sop->id}}" {{ ( $sop->id == $surat_kerja->sop_id) ? 'selected' : '' }}>{{$sop->nama_sop}}
                      @endforeach
                      </select>
                      ...
                      <option value="{{$aktivitas->id}}" {{ ( $aktivitas->id == $surat_kerja->aktivitas_id) ? 'selected' : '' }}>{{$aktivitas->nama_aktivitas}}
                      @endforeach
                      </select>
                    
                <button onclick='return confirm("Apa anda yakin data sudah benar ?")' type="submit" name="upadatedata" class="btn btn-primary" style="float:right">Ubah</button>
                </form>
                </div>
                </div>
                </div>
